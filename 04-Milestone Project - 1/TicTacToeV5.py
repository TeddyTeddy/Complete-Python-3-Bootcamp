...
def choose_turn():
    import random
    return 'user' if random.randint(0, 1) else 'computer'
...
def do_tic_tac_toe():
    ...
    players = choose_player()  # {"user":X, "player":O} or {"user":O, "player":X}
    turn = choose_turn()       # 'user' or 'computer'
    print(f'First starts {turn} with {players[turn]}')
    while True:
        if turn == 'user':
            display_board(board)
            ...
            if is_winner(players['user'], board):
                ...
                # neither a win, nor a tie, it's computers turn
                turn = 'computer'
        if turn == 'computer':
            ...
            if is_winner(players["computer"], board):
                ...
                # not a win nor a tie, it is users turn
                turn = 'user'
...
def tic_tac_toe():
    ...
    while replay:
        do_tic_tac_toe()
        replay = input("Wanna re-play the game? (y/n)").lower().startswith('y')

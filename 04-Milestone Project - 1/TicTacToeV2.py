...
def make_computer_move(players):
    ...
    for row in [0, 1, 2]:
        for column in [0, 1, 2]:
            if board[row][column] == -1:  # if the square is empty
                ...
                computer_made_the_move = is_winner(players["computer"])
        ...
        for row in [0, 1, 2]:
            for column in [0, 1, 2]:
                if board[row][column] == -1:  # if the square is empty
                    ...
                    user_to_win = is_winner(players["user"])
                    if user_to_win:
...
def tic_tac_toe():
    ...
    if players['user'] == 'X':
        ...
        while True:
            ...
            make_computer_move(players)  # makes players["computer"] move

            # display the board
            display_board()

            # if game ends, declare the winner end break
            if is_winner(players["computer"]):
